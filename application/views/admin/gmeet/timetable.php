...
if ($gmeet_value->status == 0) {
    ...
    if (!empty($gmeet_value->url)) {
?>
<a data-placement="left" href="<?php echo base_url();?>admin/gmeet/start/<?php echo $gmeet_value->id."/class"; ?>" class="btn label-success btn-xs start-mr-20" target="_blank" onclick="window.open('<?php echo $gmeet_value->url; ?>', '_blank'); return false;">
    <span class="label"><i class="fa fa-sign-in"></i> <?php echo $this->lang->line('start'); ?></span>
</a>
<?php
    } else {
?>
<button class="btn btn-danger btn-xs" disabled title="<?php echo $this->lang->line('url_not_available'); ?>">
    <i class="fa fa-exclamation-triangle"></i> <?php echo $this->lang->line('start'); ?>
</button>
<?php
    }
}
