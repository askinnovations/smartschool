...
class Gmeet extends Admin_Controller
{
    ...
    public function start($id, $type)
    {
        ...
        // Debug: Log the result to see what's returned
        log_message('debug', 'GMeet URL fetch result: ' . print_r($result, true));
        
        // Check if URL is empty or not set
        if (empty($result) || empty($result['url'])) {
            $this->session->set_flashdata('error', 'GMeet URL not found. Please check the meeting configuration.');
            redirect('admin/gmeet/timetable', 'refresh');
            return;
        }
        ...
        // Use JavaScript redirection instead of header redirect
        echo "<script type='text/javascript'>
            window.open('".$url."', '_blank');
            window.location.href = '".site_url('admin/gmeet/timetable')."';
        </script>";
        exit;
    }
}
