...
class Gmeet_model extends MY_Model
{
    ...
    public function get_urlBygmeetId($id){
        $result = $this->db->select('gmeet.url,gmeet.title,gmeet.date,gmeet.duration')
                           ->from('gmeet')
                           ->where('gmeet.id', $id)
                           ->get()
                           ->row_array();
        ...
        log_message('debug', 'GMeet URL query: ' . $this->db->last_query());
        
        if (!empty($result)) {
            // Ensure the URL is valid
            if (empty($result['url'])) {
                log_message('error', 'GMeet URL is empty for ID: ' . $id);
            }
        } else {
            log_message('error', 'No GMeet record found for ID: ' . $id);
        }
